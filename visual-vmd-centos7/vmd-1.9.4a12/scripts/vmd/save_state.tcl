...
#       $Revision: 1.48 $        $Date: 2017/06/02 20:22:31 $
#
############################################################################
#
# VMD save state $Revision: 1.48 $
#
#
...
proc save_reps {} {
  ...
  foreach mol [molinfo list] {
    ...
    for {set i 0} {$i < $numreps} {incr i} {
      ...
      set cplist {}
      set cpcount [mol clipplane num]
      for {set cp 0} {$cp < $cpcount} {incr cp} {
        set newcp {}
...
proc save_state {{file EMPTYFILE}} {
  ...
  puts $fildes "\# VMD script written by save_state \$Revision: 1.48 $"
