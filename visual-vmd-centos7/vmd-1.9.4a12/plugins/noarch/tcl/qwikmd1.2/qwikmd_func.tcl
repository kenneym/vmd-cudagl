#
# $Id: qwikmd_func.tcl,v 1.57 2017/11/15 15:57:19 jribeiro Exp $
#
#==============================================================================
proc QWIKMD::procs {} {
...
proc QWIKMD::loadTopologies {} {
    ...
    foreach topo $QWIKMD::TopList {
        if {[file exists $topo]} {
            lappend QWIKMD::topoinfo [::Toporead::read_charmm_topology $topo]
        }
    }
...
#####################################################################
proc QWIKMD::getqmIDPlot {} {
    set qmID ""
    regsub "qmmm" [lindex [split [$QWIKMD::advGui(analyze,advance,ntb) select] "."] end] "" qmID
    return $qmID
}
#####################################################################
## Plot QM Energies
...
proc QWIKMD::callQMEnergies {} {
    ...
    foreach qmID $regionlist {
        ...
        if {[info exist QWIKMD::advGui(analyze,advance,$qmID)] == 0} {
            ...
            $close entryconfigure 0 -command {
                set qmID [QWIKMD::getqmIDPlot]
                $QWIKMD::advGui(analyze,advanceplot,plot,$qmID) quit
                destroy $QWIKMD::advGui(analyze,advance,ntb).qmmm${qmID}
                set QWIKMD::advGui(analyze,advanceplot,plot,$qmID) ""
            }
            set QWIKMD::advGui(analyze,advance,plot,$qmID,xvals) [list]
            set QWIKMD::advGui(analyze,advance,plot,$qmID,evals) [list]
            set QWIKMD::advGui(analyze,advance,plot,$qmID,evalsavg) [list]
        ...
        for {set i 0} {$i < [llength $prtcl]} {incr i} {
            ...
            if {$do == 1} {
                ...
                while {[eof $logfile] != 1 } {
                    ...
                    if {[lindex $line 0]  == "QMENERGY:" && [lindex $line 1] != $prevtmstp && [lsearch [QWIKMD::format0Dec [lindex $line 2]] $regionlist] > -1} {
...
##########################################################
proc QWIKMD::reviewTopPar {reset} {
    global env
...
proc QWIKMD::GenerateNamdFiles {strct template step args outputfile} {
    ...
    if {$step > 0 || ($step == 0 && $QWIKMD::curframe > 0)} {
        set do 1
        if {$step > 0 && ([file root [lindex $QWIKMD::confFile [expr $step - 1]]] == "Minimization" || [file root [lindex $QWIKMD::confFile [expr $step - 1]]] == "QMMM-Min")} {
            set do 0
        } 
    ...
    for {set i 0 } {$i < [llength $QWIKMD::line]} {incr i} {
        if {[catch {join [lindex $QWIKMD::line $i]}] == 0} {
            if {[join [lindex $QWIKMD::line $i]] ==  "{} {}"} {
                ...
                puts $namdfile [join [lindex $QWIKMD::line $i]]
            } 
        } else {
...
proc QWIKMD::QMMMConfOpt {namdfile filename step} {
    ...
    #set filename [file rootname [file tail $QWIKMD::outPath] ]
    ...
    if {$QWIKMD::advGui(qmoptions,switchtype) != "Off"} {
        ...
        if {$index != -1} {
            QWIKMD::replaceNAMDLine "QMSwitchingType" "QMSwitchingType $QWIKMD::advGui(qmoptions,switchtype)"
        } else {
            puts $namdfile "QMSwitchingType $QWIKMD::advGui(qmoptions,switchtype)"
        }

        set index [lsearch -regexp -index 0 $QWIKMD::line (?i)QMPointChargeScheme$]
        if {$index != -1} {
            QWIKMD::replaceNAMDLine "QMPointChargeScheme" "QMPointChargeScheme $QWIKMD::advGui(qmoptions,ptchrgschm)"
        } else {
            puts $namdfile "QMPointChargeScheme $QWIKMD::advGui(qmoptions,ptchrgschm)"
        }
    } else {
        set index [lsearch -regexp -index 0 $QWIKMD::line (?i)QMSwitching$]
        if {$index != -1} {
    ...
    if {$index != -1} {
        QWIKMD::replaceNAMDLine "qmElecEmbed" "qmElecEmbed $QWIKMD::advGui(qmoptions,ptcharge)"
    } else {
        puts $namdfile "qmElecEmbed $QWIKMD::advGui(qmoptions,ptcharge)"
    }
    if {$QWIKMD::advGui(qmoptions,cmptcharge) == "On" && $QWIKMD::prepared == 1} {
    ...
    if {$QWIKMD::advGui(qmoptions,lssmode) != "Off"} {
        set lssmodeonoff on
        set lssmode dist
        if {$QWIKMD::advGui(qmoptions,lssmode) == "Center of Mass" && [info exist QWIKMD::advGui(qmtable,1,qmCOMSel)] == 1} {
            set lssmode COM
            set strlist ""
            for {set qmID 1} {$qmID <= [$QWIKMD::advGui(qmtable) size]} {incr qmID} {
    ...
    if {$index != -1} {
        QWIKMD::replaceNAMDLine "qmSoftware" "qmSoftware $QWIKMD::advGui(qmoptions,soft)"
    } else {
        puts $namdfile "qmSoftware $QWIKMD::advGui(qmoptions,soft)"
    }
    if {$QWIKMD::advGui(protocoltb,$QWIKMD::run,$step,lock) == 0} {
        if {[info exists env(QWIKMD$QWIKMD::advGui(qmoptions,soft))]} {
            set index [lsearch -regexp -index 0 $QWIKMD::line (?i)qmExecPath$]
            if {$index != -1} {
                QWIKMD::replaceNAMDLine "qmExecPath" "qmExecPath \"$env(QWIKMD$QWIKMD::advGui(qmoptions,soft))\""
            } else {
                puts $namdfile "qmExecPath \"$env(QWIKMD$QWIKMD::advGui(qmoptions,soft))\""
            }
        }
        if {$QWIKMD::prepared == 1} {
    ...
    set listaux [list]
    set QWIKMD::mdProtInfo(qmmm,cmdline) ""
    if {$firstindex != -1} {
        set listaux [lrange $QWIKMD::line 0 [expr $firstindex -1]]
    }
    foreach cmd $cmdlist {
        set str "qmConfigLine ${cmd}"
        append QWIKMD::mdProtInfo(qmmm,cmdline) "\"${cmd}\""
        if {$firstindex != -1} {
            lappend listaux ${str}
        } else {
            puts $namdfile ${str}
...
proc QWIKMD::reviewTable {tabid} {
    ...
    for {set i 0} {$i < $length} {incr i} {
        if {$QWIKMD::chains($i,0) == 0} {
            ...
            set repindex [QWIKMD::getrepnum $QWIKMD::index_cmb($QWIKMD::chains($i,1),4)]
...
proc QWIKMD::reduceSelStr {} {
    ...
    foreach chain $listChains {
        ...
        if {[llength $selRes] > 0} {
            append outstr "and chain \"$chain\"\) or "
        }
...
proc QWIKMD::reviewQMCharges {qmID} {
    ...
    if {[QWIKMD::format2Dec [expr fmod($QWIKMD::advGui(qmtable,$qmID,charge),1)]] > 0.00} {
        ...
        set return 1
    } elseif {[expr abs($QWIKMD::advGui(qmtable,$qmID,charge)) > 1] && $QWIKMD::advGui(qmoptions,soft) == "MOPAC"} {
        tk_messageBox -message "The total charges of the QM regions should be between -1 and 1 (inclusive) when using MOPAC. \
        Please add or delete more residues to reach the adequate total charge value."\
        -title "QM region charge in MOPAC" -icon info -type ok -parent $QWIKMD::selResGui 
        set return 1
    }
    return $return
}
...
## selected to balance the excess charge (in case of QM package == MOPAC)
################################################################################
proc QWIKMD::reviseQMRegion {qmID} {
    ...
    set qmmmRep [QWIKMD::getrepnum [lindex [lindex $QWIKMD::resrepname [lsearch -index 0 $QWIKMD::resrepname "qmmm"]] 1]] 
    ...
    set rep [QWIKMD::getrepnum [lindex [lindex $QWIKMD::resrepname end] 1]]
...
proc QWIKMD::rowSelection {} {
    ...
    if {$id != ""} {
        ...
        if {[regexp "QM Region Selection" [wm title $QWIKMD::selResGui]] && [llength $id] > 0} {
            ...
            mol modcolor [QWIKMD::getrepnum [lindex [lindex $QWIKMD::resrepname end] 1]] $moltop "Name"
            mol modselect [QWIKMD::getrepnum [lindex [lindex $QWIKMD::resrepname end] 1]] $moltop "$settext"
            QWIKMD::RenderChgResolution
...
proc QWIKMD::SaveInputFile {file} {
    ...
    if {$QWIKMD::run == "QM/MM"} {
        ...
        puts $ofile "set i 1"
        puts $ofile "set prtclLines \{$QWIKMD::advGui(qmoptions,ptcqmval)\}"
