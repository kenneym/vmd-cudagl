#
# $Id: qwikmd_info.tcl,v 1.10 2017/07/25 15:17:27 jribeiro Exp $
#
#==============================================================================
...
set font14 "text"
    ...
    set link {http://www.ks.uiuc.edu/Training/Tutorials/vmd/tutorial-html/node4.html#SECTION00042200000000000000}
...
proc QWIKMD::qmPlotInfo {} {

    set text1 "QM Region Energies \n\n"
    set font1 "title"
...
proc QWIKMD::protocolMDInfo {} {
    ...
    set text6 "QwikMD automatically runs 1000 steps of Energy Minimization, a temperature ramp in which the temperature of the system is raised from 60 k to the target temperature iteratively, running 1 ps per degree K. At the end of temperature ramp, an additional 1.0 ns of equilibration simulation is perform at the target temperature. During the aforementioned simulations, the backbone atoms of the proteins are restrained in space.   \n\n"
    set font6 "text"
...
proc QWIKMD::protocolMDFFInfo {} {

    set text1 "Molecular Dynamics Flexible Fitting \n\n"
    set font1 "title"
    
    set text2 "The Molecular Dynamics Flexible Fitting (MDFF) method can be used to flexibly fit atomic structures into density maps, specially cryo-electron microscopy (cryo-EM) maps. cryo-EM can capture large macromolecular complexes at different functional states usually inaccessible to X-ray crystallography techniques. However, the resolution of these maps are considerable lower when compared to the X-ray structures. Combine both structural data from different sources and resolutions allow the user to have a high resolution atomic structure representing the conformational state captured on the cryo-EM map.    \n\n"
    set font2 "text"
     
    set text3 "Despite of preparing the system with QwikMD, the user is redirected to the MDFF GUI to set up, perform and analyze the simulation.    \n\n"
    set font3 "text"
    set link {http://www.ks.uiuc.edu/Research/mdff/}
    set text [list [list $text1 $font1] [list $text2 $font2] [list $text3 $font3]  ]
    set title "Molecular Dynamics Flexible Fitting"
    return [list ${text} $link $title]
}

proc QWIKMD::protocolQMMMInfo {} {

    set text1 "Hybrid QM/MM Simulations \n\n"
    set font1 "title"
    
    set text2 "NAMD QM/MM interface extends existing NAMD features to the quantum mechanical level, presenting features that are not yet available in any QM/MM implementation. The first is the ability to execute multiple QM regions in parallel, thorough independent executions of your choice of quantum chemistry code. This allows one to account for multiple reaction centers that are known to work synergistically, for example, or even distant allosteric regulation sites and a reaction center. Investigation of processes occurring on a timescale usually not accessible by QM/MM methods can now be performed by a combination of temperature replica exchange molecular dynamics and QM/MM molecular dynamics.    \n\n"
    set font2 "text"
    
    set text3 "Another innovation comes from the fact that most QM/MM implementations have pre-defined QM and MM regions with no changes of atoms between these regions during a simulation, which would not efficiently allow the study of translocation of DNA in, e.g., nanosensors.     \n\n"
    set font3 "text"
    
    set text4 "Taking advantage of an easy-to-use Tcl based interface and capabilities integrated from VMD, NAMD QM/MM allows the update of the QM and MM regions at every step of the QM/MM simulation.     \n\n"
    set font4 "text"
    
    
    set link {http://www.ks.uiuc.edu/Research/qmmm/}
    set text [list [list $text1 $font1] [list $text2 $font2] [list $text3 $font3] [list $text4 $font4]  ]
    set title "Hybrid QM/MM Simulations"
    return [list ${text} $link $title]

}

proc QWIKMD::qmRegionsInfo {} {
    set i 1
    set text$i "QM Regions Definition \n\n"
    set font$i "title"
    
    incr i
    
    set text$i "Aiming at large macromolecular simulations that could take advantage of localized QM resolution, NAMD allows the user to set up multiple independent QM regions in the same molecular system. For example, one could study a multimeric complex that contains several active sites and have all active sites be calculated with a chosen QC software simultaneously. Each active site would be calculated independently of all others, by its own execution of the QC software, keeping the calculation cost low and without impacting the overall efficiency of the simulation, since all QM regions would be calculated in parallel.    \n\n"
    set font$i "text"
    
    incr i
    
    set text$i "Identifying the different QM regions and which atoms belong to each one of them can be simply accomplished in the input PDB file. Since each region can contain different sets of molecules, their charges and multiplicities are indicated separately.     \n\n"
    set font$i "text"
    
    incr i
    
    set text$i "For simulations of large systems that are distributed across several computer nodes, one can control how many independent QM regions are calculated in each node. This would prevent large simulations from running out of memory if two or more large QM regions are placed in the same node (see keyword \"qmSimsPerNode\").     \n\n"
    set font$i "text"
    
    incr i
    
    set text$i "Link Atom Charge and Charge Groups    \n\n"
    set font$i "subtitle"
    
    incr i
    
    set text$i "Along with the gradient over all QM atoms, NAMD can also use the partial charge derived from the QC calculation to update the charge distribution of atoms in the QM region. \
    When a QM/MM bond exists, however, part of the charge of the region will be placed on the link atom, and in order to keep the charge of the QM region constant, the link atom charge is \
    re-distributed on the QM region. This seemingly simple mechanism can cause problems unless special care is be taken when deciding which bond will mark the division of QM and MM regions.    \n\n"
    set font$i "text"
    
    incr i

    set text$i "Many force fields divide the topologies of biomolecules in \"charge groups\". What this means is that not only will the partial charges of all atoms of a \
    molecule add up to the whole number that represents the charge of the molecule, they will also add up to whole numbers in sub groups of atoms (look for the words \"GROUP statements\" \
        in this link to see an example). Therefore, one needs to make sure that the chosen QM/MM bond(s) sits in between \"charge groups\", so the total sum of partial charges of atoms \
defining a QM region is a whole number. This is especially important in order to keep the total charge of the system constant. Since the QC calculation will always distribute a whole \
charge over all atoms of a system (QM atoms plus a link atom), if the partial charge of QM atoms is not initially a whole number, it will be forced into a whole number after the first \
QC step, where the charge of the link atom is distributed over the QM region. This will create a mismatch between QM and MM charges, changing the total charge of the entire system (QM plus MM regions).    \n\n"
    set font$i "text"
    
    incr i

    set text$i "Therefore, any charge placed on link atoms can be redistributed to the QM atoms with no change in total system charge. However, if bonds \
    2 and 3 are chosen for the QM/MM bond, the charge of the MM region would be +1.16, while the charge of the QM region would be -1.16. Since the QC calculation would \
    place a pre-determined whole charge on the region (-1, in this case), the updated charge of the QM region after the first simulation step would change the total charge of the \
    system to +0.16, in this example.    \n\n"
    set font$i "text"
    
    incr i

    set text$i "Multiplicity of the QM region    \n\n"
    set font$i "subtitle"
    
    incr i
    
    set text$i "This is needed for proper construction of ORCA's input file. Each string must be composed of the QM region ID and its multiplicity.    \n\n"
    set font$i "text"
    ...
    for {set j 1} {$j < $i} {incr j} {
        set a text$j
        set b font$j
        lappend alltext [list [subst $$a] [subst $$b]]
    }
    set title "QM Regions Definition"
    return [list ${alltext} $link $title]

}

proc QWIKMD::qmOptionsInfo {} {
    set i 1

    set text$i "QM Options \n\n"
    set font$i "title"
    
    incr i

    set text$i "QM Software    \n\n"
    set font$i "subtitle"
    
    incr i
    
    set text$i "Indicates which QM software should be used (keyword \"qmSoftware\"). In case the user wants to apply another QM code, this can be done by using the \"custom\" qmSoftware. In this case, NAMD will call the executable defined in the qmExecPath variable and will give it one argument: the full path to the input file.    \n\n"
    set font$i "text"
    
    incr i

    set text$i "Live Solv. Mode(Live Solvent Mode)    \n\n"
    set font$i "subtitle"

    incr i

    set text$i "NAMD automatically keeps track of the solvent molecules for all QM Groups, and will exchange classical solvent molecules with QM solvent molecules every \"QMLSSFreq\" steps.  In all cases, the closest solvent molecules are selected, and if a classical solvent molecule is closer than a QM one, they are swapped.\
     Distance: This mode will use the smallest distance between a solvent atom and a non-solvent QM atom to sort solvent molecules. This is best used when the non-solvent QM atoms form irregular volumes (when the Center of Mass (COM) is not very representative), and/or volumes with high solvent accessibility (such as a drug, or\
      a small peptide, in solution). COM: This mode will sort solvent molecules based on Center Of Mass distance between the solvent COM and the COM of a selection for each QM group (see \"QMLSSRef\" keyword). Best used with small QM regions that have limited solvent accessibility, such as an active site.      \n\n"
    set font$i "text"
    
    incr i
    set text$i "Point Charges    \n\n"
    set font$i "subtitle"
    
    incr i

    set text$i "Electrostatic interactions between QM and MM atoms deserve a more detailed discussion due to the abundance and diversity of available alternatives. The first decision to be made is whether there will be electrostatic interactions between the two portions of a system, QM and MM. In the \"mechanical embedding\" scheme, only positions and elements of atoms in the QM region are passed on to the chosen QC software for energy and force calculations. This way, QM and MM atoms share only Van der Waals interactions.     \n\n"
    set font$i "text"
    
    incr i

    set text$i "In the \"electrostatic embedding\" scheme, on the other hand, the partial charges of MM atoms surrounding all QM atoms are used to approximate the electrostatic environment where QM atoms are found  (keyword \"qmElecEmbed\").This process can be customized in a variety of ways, the first of which is deciding if a smoothing function will be used to avoid an abrupt decay in electrostatic force due to the cutoff used in the selection of surrounding point charges.      \n\n"
    set font$i "text"
    
    incr i

    set text$i "Classical point charge utilization can be further customized by choosing which smoothing function will be used, and if the total charge of selected partial charges should be modified to (A) have a whole charge or (B) have a complementary charge to that of the QM region, so that the sum of charges from QM atoms and classical partial charges add to zero.      \n\n"
    set font$i "text"
    
    incr i

    set text$i "With electrostatic embedding, QM atoms are influenced by the charges in the classical region. In order to balance the forces acting on the system, NAMD uses partial charges for the QM atoms to calculate the electrostatic interaction with classical point charges. There are two possibilities for the origin of the QM partial charges: the original partial charges found in the force field parameter\
     files can be used, or updated partial charges can be gathered at each step form the QC software output (controllable through the \"qmChargeMode\" keyword). The continuous update in charge distribution allows for a partial re-parameterization of the targeted molecule at each time step, which can lead to an improved description of the interactions of a ligand as it repositions over the surface of a protein, for example, or as it moves through a membrane.      \n\n"
    set font$i "text"

    incr i

    set text$i "In case PME is activated by the user, NAMD will automatically apply the necessary corrections to handle the QM region, allowing it to be influenced by long range interactions from the entire system.      \n\n"
    set font$i "text"

    incr i

    set text$i "Point Charge Scheme    \n\n"
    set font$i "subtitle"
    
    incr i

    set text$i "Several methods have been proposed to handle this situation, and the QM/MM interface developed here implements the most widely accepted ones. One can be chosen using the \"qmBondScheme\" keyword. \
    In all implemented methods, the classical atom participating in the QM/MM bond (MM1 atom) does not have its partial charge passed on to the QC software, since this would create excessive repulsion \
    (or attraction) on the link atom. This is, in fact, the entirety of the \"Z1\" method: ignoring the partial charge of the MM1 atom. Analogously, \"Z2\" and \"Z3\" ignore all partial charges up to \
    MM2 and MM3 atoms, respectively.     \n\n"
    set font$i "text"
    
    incr i

    set text$i "The Redistributed Charge and Dipole (RCD) method is more elaborate, as it rearranges the partial charge of the MM1 atom (indicated as q) so that the total charge of the region is maintained \
    as well as the dipole moments of the bonds between MM1 and MM2 atoms. This is done by creating \"virtual\" point charges, which are passed on to the QC software as if they represented partial charges of \
    classical atoms. More specifically, the RCD method creates a virtual point charge in the middle of all MM1-MM2 bonds with a charge of 2*q/n, where n is the number of MM2 atoms connected to MM1, and also \
    subtracts a charge q/n from each of the MM2 atoms, so that the total charge of the region remains constant while approximating the dipole moment of the MM1-MM2 bonds. This way there will be no point \
    charge placed at the position of the MM1 atom, but its partial charge is not simply removed, it is redistributed.     \n\n"
    set font$i "text"
    
    incr i

    set text$i "A similar approach is taken by the Charge Shifting (CS) method. In this case, the MM1 partial charge is equally distributed across the MM2 atoms, and two virtual point charges \
    are placed along the direction of the MM1-MM2 bond, one before the MM2 atom and one after, each one with a charge of +q/n and -q/n respectively. This method will also keep the total charge \
    of the region constant while trying to preserve the local dipoles formed by all MM1-MM2 bonds.     \n\n"
    set font$i "text"
    
    incr i

    set text$i "QM Command line    \n\n"
    set font$i "subtitle"
    
    incr i

    set text$i "The string passed to \"qmConfigLine\" will be copied and pasted at the very beginning of the configuration file for the chosen QM software if either ORCA or MOPAC are selected.      \n\n"
    set font$i "text"
    ...
    set link {http://www.ks.uiuc.edu/Research/qmmm/}
    #set text [list [list $text1 $font1] [list $text2 $font2] [list $text3 $font3] [list $text4 $font4] [list $text5 $font5] [list $text6 $font6] [list $text7 $font7] [list $text8 $font8] [list $text9 $font9] ]
    set title "QM Options"
    return [list ${alltext} $link $title]
...
proc QWIKMD::ChiralityInfo {} {
    ...
    set text2 "All amino acids but glycine have at least one chiral center at C\u03B1.\
...
proc QWIKMD::renderInfo {} {
    ...
    set text2 "Produce a paper-quality image is sometimes a laborious and complex task to achieve. Within the color scheme and render \
    section, one can select the color palette and image size that produces the best representation of the molecular system. One can also\
    select the resolution of the materials, by selection one of the option in the \"Resolution\" drop-down menu.     \n\n"
    set font2 "text"

    set text3 "When a color scheme is selected rather than \"VMD Classic\", QwikMD also sets the following values:\n\t\
    shadows on\n\tambientocclusion on\n\tnearclip 0.010000\n\tcuedensity 0.20\n\tand the rendermode GLSL.    \n\n"
    set font3 "text"

    set text4 "Material Resolution\n\tLow - VMD Default Resolution\n\tMedium - Default Resolution * 2\n\tHigh - Default Resolution * 3\n\tMax - Default Resolution * 4     \n\n"
    set font4 "text"

    set link {http://www.ks.uiuc.edu/Training/Tutorials/vmd-imgmv/imgmv/imgmv-tutorial.pdf}
    set text [list [list $text1 $font1] [list $text2 $font2] [list $text3 $font3] [list $text4 $font4]]
