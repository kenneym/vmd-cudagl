...
# 1.8                     2013-12-20
#----------------------------------------------------------------------
# $Id: parseFEP.tcl,v 1.15 2017/12/18 21:33:27 johns Exp $
#----------------------------------------------------------------------
...
# (3) Bugs of reading fepout files in BAR_estimator and inaccuracy_estimation have been removed.
#######################################################################

package require exectool 1.2
package require multiplot
package provide parsefep 2.0
...
##################################################################
# Routine for reading NAMD FEP output file (Letitia)
##################################################################
...
proc ::ParseFEP::namdparse {  } {
   ...
   # Update the parameter (Letitia: A lot of calculations are done using ::ParseFEP::kT, whose value is never updated for T!=300)
   ...
   # Inaccuracy analysis (only possible when backward file also exists)
   if  { $::ParseFEP::fepbofile != "" }  { ::ParseFEP::inaccuracy_estimation }
...
proc ::ParseFEP::FEP_formula {window mean_deltaG fororback nfepdata fepdata fepdata_energy1 fepdata_energy2} {
   ...
   # evaluate some data...(Letitia: I don't know what are they. Jsut clean up the code.)
   set instant_accum 0. 
   ...
   # Evaluate even more data... (Letitia: I can only recognize the free energy term average_b...)
...
proc ::ParseFEP::sos_estimate {} {
   ...
   puts $fid "ParseFEP: Simple overlap sampling on $::ParseFEP::fepofile and $::ParseFEP::fepbofile runs"
		...
		if { $::ParseFEP::fepbofile != "" } {
			...
			while { $index < 16 * $page     } {
				...
				if { [file exists [format "file%d.dat" $file_index] ] }  {
                                        ...
                                        set lambda2 [format "%8.4f"    [lindex $::ParseFEP::fwd_lambda2 $index]
